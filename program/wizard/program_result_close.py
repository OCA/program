...
class program_result_close(orm.TransientModel):
    ...
    _name = 'program.result.close'
    _description = "Result Mass Closer"
    _columns = {
        'line_ids': fields.many2many(
            'program.result.close.line',
            'program_result_close_rel',
            string='Results',
        ),
    }
    ...
    def _build_result_ids(self, cr, uid, context=None):
        if context is None:
            context = {}
        res = []
        result_line_pool = self.pool['program.result.close.line']
    ...
    def run(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context).line_ids:
            if line.is_closable:
                ...
                non_closable_ids.append(line.result_id)
        if non_closable_ids:
            # Don't do the following in a generator as it doesn't translate
            culprits = set()
            for r in non_closable_ids:
                culprits.add((r.name, _(dict(STATES)[r.state])))
            raise orm.except_orm(
                _('Error'),
                _("The following results cannot be closed or "
